...
import com.bytezone.network.OnionRouter;
...
public class NetworkGame extends Game
{
    ...
    public void startNetworkGame (GameClient gameClient)
    {
        ...
        // اتصال به سرور از طریق OnionRouter
        ...
        onionRouter.sendOnionMessage("Connecting to game server...");

        initialiseBoard ();
        gameClient.sendName (gameClient.getName ());          // بازیکنان نام‌های خود را رد و بدل می‌کنند

        if (gameMode == GameMode.SERVER)
            players[SERVER].setName (gameClient.getName ());    // نام خود را تنظیم کنید
        else
        {
            players[CLIENT].setName (gameClient.getName ());    // نام خود را تنظیم کنید
    ...
    public void receiveMove (byte[] buffer)
    {
        ...
        // بررسی مهره‌های گرفته شده
        if (toPoint.size () > 0)
        {
            Checker checker = toPoint.peek ();
            if (checker.getOwner () == opponent)
            {
                capturedChecker = toPoint.pop ();           // آن را از نقطه حذف می‌کنیم
                assert toPoint.size () == 0;                // نقطه اکنون خالی است
            }
        }

        // پردازش حرکت و رسم موقعیت جدید
    ...
    {
        board.reversePerspective ();
        gameClient.sendCommand (GameClient.FINISH_TURN);    // به حریف اعلام می‌شود که نوبت اوست
    }


    public void sendMessage (String message)
    {
        // ارسال پیام چت به بازی Peer-to-Peer
    ...
    protected Optional<Move> doPointClick (Point sourcePoint)
    {
        ...
        if (opt.isPresent ())
            gameClient.sendMove (opt.get ());  // حرکت به حریف ارسال می‌شود
